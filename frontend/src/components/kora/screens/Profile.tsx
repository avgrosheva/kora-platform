    ...
    <div className="relative z-10 max-w-[820px] px-9 pb-24 pt-10">
      ...
      <Panel className="kora-rise kora-d1 mb-[18px] overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-[18px] p-[22px]">
          <div>
            ...
            <div className="text-[13px] text-fg-secondary">{user.email}</div>
          </div>
          <div>
            <FieldLabel>FULL NAME</FieldLabel>
            <div className="text-[13px] text-fg-secondary">{user.fullName ?? '—'}</div>
          </div>
          <div>
            <FieldLabel>ACCOUNT CREATED</FieldLabel>
            <div className="font-mono text-[13px] text-fg-secondary">{user.createdAt}</div>
          </div>
          <div>
            <FieldLabel>STATUS</FieldLabel>
            <div className="text-[13px] text-fg-secondary">{user.isActive ? 'Active' : 'Inactive'}</div>
          </div>
        </div>
        <div className="border-t border-white/[0.055] px-[22px] py-[18px]">
          <div className="rounded-[9px] border border-white/[0.09] bg-white/[0.025] px-4 py-3 text-[12.5px] leading-relaxed text-fg-dim [text-wrap:pretty]">
      ...
      <section className="kora-rise kora-d2 rounded-[14px] border border-danger/[0.22] bg-gradient-to-b from-danger/[0.05] to-white/[0.008] p-[22px]">
        <div className="mb-2.5 flex items-center gap-[9px]">
          ...
          <span className="font-mono text-[10px] tracking-label text-danger-soft">SESSION</span>
        </div>
