    ...
    <div>
      ...
      {insufficient ? (
        <section className="relative mb-5 overflow-hidden rounded-2xl border border-white/[0.07] bg-hero-warn px-[30px] py-[52px] text-center">
          ...
          <div className="mb-3.5 font-mono text-[10px] tracking-[0.18em] text-fg-muted">NO SCORE YET</div>
          <h2 className="m-0 mb-2.5 text-[22px] font-medium tracking-[-0.5px] text-warn-wash">
            Insufficient evidence for a composite score
          </h2>
          <p className="mx-auto m-0 max-w-[520px] text-[13px] leading-relaxed text-fg-dim [text-wrap:pretty]">
        ...
        <section className="mb-5 overflow-hidden rounded-2xl border border-white/[0.07] bg-hero-accent px-[30px] py-11 text-center">
          <div className="mb-4 font-mono text-[10px] tracking-[0.18em] text-accent-ghost">INVESTMENT SCORE</div>
          ...
          <p className="m-0 mt-3.5 text-[13px] text-fg-dim">
      ...
      {score.reasoning && (
        <Panel className="mb-5 px-[22px] py-5">
          ...
          <p className="m-0 text-[13px] leading-relaxed text-fg-tertiary [text-wrap:pretty]">{score.reasoning}</p>
        </Panel>
      )}
            ...
            {Object.entries(score.category_breakdown).map(([key, entry]) => (
              <div key={key}>
                <div className="mb-2 flex items-baseline justify-between">
                  <span className="text-[13px] text-fg-secondary">{CATEGORY_LABELS[key] ?? key}</span>
