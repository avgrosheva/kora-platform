  ...
  if (!report) {
    return (
      <EmptyState
        ...
        blurb="Generates the evidence-grounded report: verified facts, red flags from deterministic checks, and founder questions — on top of the AI-written narrative."
        action={
  ...
  return (
    <div>
      ...
      <Panel className="mb-4 px-[26px] py-6">
        ...
        <p className="m-0 mb-6 text-[13.5px] leading-[1.7] text-fg-tertiary [text-wrap:pretty]">{report.executive_summary}</p>
        {report.verified_facts.length > 0 && (
          <div className="grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-5 border-t border-white/[0.055] pt-5">
            {report.verified_facts.map((fact) => (
              <div key={fact.label}>
                <div className="mb-2 font-mono text-[9.5px] tracking-label text-fg-faint">{fact.label.toUpperCase()}</div>
      ...
      {report.red_flags.length > 0 && (
        <Panel className="mb-4 px-6 py-[22px]">
          <div className="mb-4 flex items-center gap-2.5">
            ...
            <h2 className="text-sm font-semibold text-danger-pale">Red flags ({report.red_flags.length})</h2>
          </div>
            ...
            {report.red_flags.map((flag, i) => (
              ...
              >
                <div className="mb-1.5 flex flex-wrap items-center gap-2.5">
                  ...
                  <span className="text-[13.5px] font-medium">{flag.title}</span>
                </div>
                <p className="m-0 text-xs leading-relaxed text-fg-muted [text-wrap:pretty]">{flag.description}</p>
              </div>
            ))}
          </div>
        </Panel>
      ...
      {report.founder_questions.length > 0 && (
        <Panel className="mb-4 px-6 py-[22px]">
          <h2 className="mb-4 text-sm font-semibold">Questions for the founders</h2>
          <div className="flex flex-col gap-3">
            {report.founder_questions.map((q, i) => (
              <div key={i} className="flex items-start gap-3">
                ...
                <span className="text-[13px] leading-relaxed text-fg-tertiary [text-wrap:pretty]">{q.question}</span>
              </div>
            ))}
          </div>
        </Panel>
