...
import { GapChip, Meter, Panel, SectionLabel } from "../../primitives";
  ...
  return (
    <div>
      <Panel className="mb-4 px-[26px] py-6">
        <div className="mb-[26px] flex flex-wrap items-start justify-between gap-8">
          <div className="max-w-[640px]">
            <h2 className="mb-2 text-[15px] font-semibold">Analysis coverage</h2>
            <p className="m-0 text-[12.5px] leading-relaxed text-fg-dim [text-wrap:pretty]">
          ...
          <div className="text-right">
            ...
            {missingInfo && (
              <div className="font-mono text-[9.5px] tracking-label text-fg-faint">
          ...
          {categories.map((cat, i) => (
            <div key={cat.name}>
              <div className="mb-2 flex items-baseline justify-between">
                <span className="text-[13px] text-fg-secondary">{cat.name}</span>
                <span className={"font-mono text-[11.5px] " + textToneFor(cat.pct)}>{cat.filled}/{cat.total}</span>
              </div>
      ...
      <Panel className="px-[26px] py-6">
        <h2 className="mb-1.5 text-[15px] font-semibold">Missing information by category</h2>
        <p className="m-0 mb-6 text-[12.5px] text-fg-dim">
          What to request for each gap, and why it matters — not just a list of field names.
        </p>

        {grouped.length === 0 ? (
          <div className="py-4 text-center text-[12.5px] text-fg-dim">Nothing missing — every checklist field was found.</div>
        ) : (
          grouped.map(([category, items]) => (
            <div key={category} className="mb-[22px] last:mb-0">
              <div className="mb-3 font-mono text-[9.5px] tracking-label text-accent-ghost">
              ...
              <div className="flex flex-col gap-[9px]">
                {items.map((item, i) => (
                  ...
                  >
                    <span className="w-[130px] shrink-0 font-mono text-[10.5px] text-fg-secondary">{item.field_name.replace(/_/g, " ")}</span>
                    <span className="text-[12.5px] text-fg-muted [text-wrap:pretty]">{item.recommended_request ?? "—"}</span>
                  </div>
                ))}
              </div>
            </div>
