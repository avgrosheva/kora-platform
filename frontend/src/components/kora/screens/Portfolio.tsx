    ...
    <div className="relative z-10 max-w-[1360px] px-9 pb-24 pt-10">
      ...
      <Panel className="kora-rise kora-d2 mb-4 overflow-hidden">
        <PanelHeader
          ...
          aside={<span className="font-mono text-[11px] tracking-badge text-fg-faint">{rows.length} RESULT{rows.length === 1 ? '' : 'S'}</span>}
        />
        <div className="grid grid-cols-[2fr_0.8fr_1.2fr_1.5fr_0.9fr] gap-4 border-b border-white/[0.04] px-5 py-2.5 font-mono text-[10.5px] tracking-label text-fg-faint">
        ...
        {rows.map((row) => (
          ...
          >
            <div>
              <div className="mb-[3px] text-[15px] font-medium">{row.companyName ?? row.filename}</div>
              <div className="font-mono text-[12px] text-fg-dim">{row.filename}</div>
            </div>
            <div className={'font-mono text-[14px] ' + (row.score == null ? 'text-warn' : 'text-fg')}>{row.score ?? '—'}</div>
            <div>
              ...
              <div className="font-mono text-[11px] text-fg-dim">{row.coverage != null ? row.coverage + '%' : '—'}</div>
            </div>
      ...
      <div className="grid grid-cols-[1.35fr_1fr] gap-4">
        <Panel className="kora-rise kora-d3 px-[22px] pb-[18px] pt-5">
          <div className="mb-6 flex items-baseline justify-between">
            <h2 className="text-[14.5px] font-semibold">Portfolio health</h2>
            <span className="font-mono text-[10.5px] tracking-label text-fg-faint">DOCUMENTS BY SIGNAL</span>
          </div>
          ...
          <div className="mt-3 flex">
            {health.map((bucket) => (
              <div key={bucket.label} className={'flex-1 text-center font-mono text-[11px] ' + (bucket.count > 0 ? 'text-fg-quiet' : 'text-fg-faint')}>
        ...
        <Panel className="kora-rise kora-d4 flex flex-col px-[22px] py-5">
          <h2 className="mb-[18px] text-[14.5px] font-semibold">Top scored documents</h2>
          {topScored.length === 0 ? (
            <div className="flex flex-1 flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-white/[0.08] px-5 py-[26px] text-center">
              ...
              <div className="text-[14px] text-fg-quiet">No scored documents yet</div>
              <div className="max-w-[230px] text-[12.5px] text-fg-faint [text-wrap:pretty]">
            ...
            <ul className="m-0 flex list-none flex-col gap-2.5 p-0">
              {topScored.map((doc) => (
                <li key={doc.id} className="flex items-center justify-between rounded-[10px] border border-white/[0.06] bg-white/[0.02] px-3.5 py-3">
                  ...
                  <span className="font-mono text-[16px] text-good">{doc.score}</span>
                </li>
              ))}
            </ul>
          )}
  ...
  return (
    <div className="relative z-10 flex min-h-[620px] items-center justify-center px-9 py-16">
      <div className="kora-rise max-w-[520px] text-center">
        ...
        <div className="mb-3.5 font-mono text-[11px] tracking-[0.18em] text-fg-faint">NO ORGANIZATIONS</div>
        <h1 className="m-0 mb-3.5 text-[27px] font-semibold tracking-[-0.7px]">
          Create your first organization to get started
        </h1>
        <p className="m-0 mb-[26px] text-[14.5px] leading-relaxed text-fg-dim [text-wrap:pretty]">
